<div id="mtopLeft" class="computer ui-btn ui-shadow ui-btn-corner-all ui-btn-up-c"> 
  ...
  <div>
    <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://volebnikalkulacka.cz" data-via="KohoVolitEU" data-lang="{$text.result_tiwtter_data_lang}" data-size="large" data-count="none" data-text="{$text.result_twitter_text_1}{if $results[0].sex == 'm'}{$text.result_twitter_text_2_m}{else}{$text.result_twitter_text_2_f}{/if} {$results[0].last_name}({$results[0].party},{$results[0].result_percent}%){$result_twitter_text_3}{$results[$results_count-1].last_name}({$results[$results_count-1].party},{$results[$results_count-1].result_percent}%){$text.result_twitter_hashtags}">{$text.result_twitter_share}</a>
   </div>

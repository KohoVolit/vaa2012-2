                              ...
                              <tr id="who-row-{$candidate['id']}" class="who-row">
                                <td class="weightsel-td">
                                  <label class="weightsel-button btn btn-lg btn-block" for="who-{$candidate['id']}" id="label-who-{$candidate['id']}">
                                    <input type="checkbox" name="who[]" id="who-{$candidate['id']}" value="{$candidate['id']}"/> <strong>{$candidate['name']}</strong> <small>({$candidate['party_abbreviation']}<span class="hidden-xs"> - {$candidate['party']|truncate:75:"..."}</span>)</small>
                                  </label>
                                </td>
                              </tr>
                          {/foreach}

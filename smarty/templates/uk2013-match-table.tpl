	  ...
	  <div class="question ui-body ui-body-{$partner['swatch_question_body']}">	  
	    <ul data-role="listview" data-theme="{$partner['swatch_question_body']}" data-filter="true" data-filter-placeholder="{$text.result_filter}" data-filter-theme="e">
	      {foreach $results as $key=>$result}
	        <li data-theme="c"><a href="../compare.php?{$query_string}&id={$result.id}&match={$result.result_percent}" data-rel="dialog" ><img src="../../image/1x1.png" class="uk2013-sprite uk2013-sprite-{$result.friendly_name}-80" alt="{$result.name}"/>
	        <h3>{$result.name|truncate:35:"...":true}</h3>
	        <div class="small">{$result.first_name|truncate:45:"...":true}</div>
	        <p class="ui-li-aside">

...
export default {
  async asyncData ({ $content }) {
    const answers = await $content('answers_young').only(['id']).fetch()
    return { answers }
  },
  ...
  head: function() {
    return {
      title: 'Mladí volí, Volební kalkulačka 2021 - výběr stran',
      meta: [
        {
          hid: 'compare',
          name: 'Výběr stran',
          description: 'Mladí volí, Volební kalkulačka 2021 - výběr stran'
        }
      ]
    }
  },

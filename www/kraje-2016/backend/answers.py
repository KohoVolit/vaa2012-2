...
for k in constituencies:
    questions_url = constituencies[k]['questions']
    answers_url = constituencies[k]['answers']
    ...
    for row in csv.reader(csvio):
        if i == 0:
            ...
            voter = {
                ...
                'constituency': row[1].strip(),
                'friendly_name': slugify(row[2].strip()),
                'picture': settings['picture_prepend'] + row[6].strip() +  settings['picture_append'],
                'small_picture': settings['small_picture_prepend'] + row[6].strip() +  settings['small_picture_append']
            }
    ...
    # length of row to number of questions
    def len2nq(length):
        return (length-4) / 2
    ...
    for key in voters:
    #    print(key)
        if voters[key]['constituency'] == code:
            try:
                voters[key]['votes']
        #        print(voters)
            except:

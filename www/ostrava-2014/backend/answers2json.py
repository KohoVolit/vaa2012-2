...
with open('source/answers.tsv','r') as f:
  ...
  for row in csvreader:
    if i == 0:
      ...
      for j in range(0,nq):
        col = (4 + j)
        questions[col] = re.search('(^|\\s)([0-9]+)', row[col]).group(0).strip() #re.search('\d*', row[col]).group(0)
      #print(questions)
    else:
      try:
        votes = {}
...
#            details[questions[key]] = {}
          if row[key+1].strip() != "":
            details[voter_id][questions[key]] = row[key+44].strip()
        #print votes
